...
- `cache/<topic_id>/<cache_key>/summary/*.json`: cached `esummary` batch payloads
...
- If only part of the cache exists, the script reuses cached PMID lists and batch payloads, then fetches only missing pieces.
