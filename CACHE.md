...
- `cache/<topic_id>/<cache_key>/raw_records.json`: cached normalized topic records before filtering
- `cache/<topic_id>/<cache_key>/filtered_records.json`: cached topic records kept after filtering
- `cache/<topic_id>/<cache_key>/review_records.json`: cached kept records flagged for manual review
- `cache/<topic_id>/<cache_key>/meta.json`: cache metadata for the topic run

## Resume behavior

- If `raw_records.json`, `filtered_records.json`, `review_records.json`, and `pmids.json` already exist for the same topic/query/batch parameters and the same `max-records-per-topic` setting, the script reuses them and skips network retrieval for that topic.
...
- `run_manifest.json`: run-level retrieval parameters and counts.
- `topic_summary.csv`: per-topic PMID, raw, filtered, and review counts.
- `retrieval_quality_summary.csv`: per-topic metadata quality counts and aggregated filter-reason counts.
